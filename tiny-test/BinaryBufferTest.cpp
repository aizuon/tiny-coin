...
TEST(BinaryBufferTest, PrimitiveReadWrite)
{
	...
	buffer.Write(b_real);
	bool b_read = false;
	EXPECT_TRUE(buffer.Read(b_read));
	EXPECT_EQ(b_real, b_read);

	uint8_t u8 = 3;
	buffer.Write(u8);
	uint8_t u8_read = 0;
	EXPECT_TRUE(buffer.Read(u8_read));
	EXPECT_EQ(u8, u8_read);

	int8_t i8 = -5;
	buffer.Write(i8);
	int8_t i8_read = 0;
	EXPECT_TRUE(buffer.Read(i8_read));
	EXPECT_EQ(i8, i8_read);

	uint16_t u16 = 10000;
	buffer.Write(u16);
	uint16_t u16_read = 0;
	EXPECT_TRUE(buffer.Read(u16_read));
	EXPECT_EQ(u16, u16_read);

	int16_t i16 = -5000;
	buffer.Write(i16);
	int16_t i16_read = 0;
	EXPECT_TRUE(buffer.Read(i16_read));
	EXPECT_EQ(i16, i16_read);

	uint32_t ui32 = 7000000;
	buffer.Write(ui32);
	uint32_t ui32_read = 0;
	EXPECT_TRUE(buffer.Read(ui32_read));
	EXPECT_EQ(ui32, ui32_read);

	int32_t i32 = -3000000;
	buffer.Write(i32);
	int32_t i32_read = 0;
	EXPECT_TRUE(buffer.Read(i32_read));
	EXPECT_EQ(i32, i32_read);

	uint64_t ui64 = 4000000000;
	buffer.Write(ui64);
	uint64_t ui64_read = 0;
	EXPECT_TRUE(buffer.Read(ui64_read));
...
TEST(BinaryBufferTest, StringReadWrite)
{
	...
	buffer.Write(str);
	std::string str_read;
	EXPECT_TRUE(buffer.Read(str_read));
...
TEST(BinaryBufferTest, VectorReadWrite)
{
	...
	std::vector<int32_t> i32 = { -3000000, -4000000, -5000000 };
	buffer.Write(i32);
